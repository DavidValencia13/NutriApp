...
class GeneradorMenuGroq {
  ...
  async generar({ perfilPaciente, alimentosDisponibles }) {
    ...
    try {
      ...
      if (error instanceof GroqRateLimitError) {
        const espera = error.retryAfterSeconds
          ? ` Espera ${error.retryAfterSeconds} segundos antes de intentarlo de nuevo.`
          : " Revisa los límites de uso del proyecto en Groq Console antes de intentarlo de nuevo.";
        throw new ServicioExternoError(
          `Se alcanzó el límite de Groq.${espera}`,
          429,
        );
      }
      throw new ServicioExternoError(
    ...
    try {
      ...
      console.error("Menú de la IA rechazado por formato inválido");
      throw error;
    }
    return resultado;
  }
